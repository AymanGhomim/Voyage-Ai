...
export function Dashboard() {
  ...
  if (state.status === "loading") {
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="h-8 w-8 rounded-full gradient-aurora animate-pulse shadow-glow" />
      </div>
    );
  }
  ...
  if (state.status === "unauthenticated") {
    return (
      <div className="relative flex min-h-screen items-center justify-center overflow-hidden">
        ...
        <div aria-hidden className="pointer-events-none absolute -top-40 left-1/2 h-[600px] w-[900px] -translate-x-1/2 rounded-full opacity-50 blur-3xl gradient-aurora" />
        <motion.div
        ...
        >
          ...
          <button
            ...
            className="mt-8 inline-flex w-full items-center justify-center gap-2 rounded-full px-6 py-3.5 text-sm font-semibold text-primary-foreground shadow-glow transition hover:scale-[1.02] active:scale-[0.99]"
            style={{ background: "var(--gradient-aurora)" }}
          >
            <LogIn className="h-4 w-4" /> Sign in to Voyage AI
          </button>
          <Link to="/" className="mt-4 inline-block text-xs text-muted-foreground hover:text-foreground transition">
            ← Back to home
          </Link>
        </motion.div>
        <AnimatePresence>
  ...
  const handleAddItinerary = (days: Day[]) => {
    setTripList((prev) => prev.map((t) => (t.id === activeId ? { ...t, days } : t)));
    setAiOpen(false);
    toast.success("Itinerary added! 🗺️", { description: "Your AI-curated trip plan is ready." });
  };
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      <div aria-hidden className="pointer-events-none fixed -top-40 right-0 h-[600px] w-[800px] rounded-full opacity-40 blur-3xl gradient-aurora" />
      ...
      <header className="relative z-10 flex items-center justify-between px-4 py-3 lg:hidden">
        <Link to="/" className="flex items-center gap-2">
          ...
          <span className="text-lg font-semibold">Voyage<span className="text-gradient-aurora">AI</span></span>
        </Link>
      ...
      <div className="relative z-10 mx-auto flex max-w-[1600px] gap-4 p-4 pt-0 lg:pt-4">
        ...
        <main className="flex-1 min-w-0">
          <TripWorkspace trip={trip} onOpenAI={() => setAiOpen(true)} />
        </main>
      ...
      <AnimatePresence>
        {aiOpen && (
          <AIAssistantModal onClose={() => setAiOpen(false)} onAddToTrip={handleAddItinerary} hasDays={trip.days.length > 0} />
        )}
...
/* ─── Sidebar ─────────────────────────────────────── */
function SidebarNav({
  trips, activeId, onSelect, onNew, user,
}: {
  trips: Trip[];
  activeId: string;
  ...
  const handleLogout = () => {
    logout();
    toast("Signed out. See you next adventure! ✈️");
  };
  ...
  return (
    <aside className="glass flex w-72 shrink-0 flex-col rounded-3xl p-4 h-[calc(100vh-2rem)] lg:sticky lg:top-4">
      ...
      <button
        type="button"
        onClick={onNew}
        className="mt-3 inline-flex items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-semibold text-primary-foreground shadow-glow transition hover:scale-[1.01] active:scale-[0.99]"
        style={{ background: "var(--gradient-aurora)" }}
      >
            ...
            filtered.map((t) => {
              ...
              return (
                <motion.button
                  ...
                  className={`group relative flex w-full items-center gap-3 rounded-2xl p-2 pr-3 text-left transition ${
      ...
      <div className="mt-3 border-t border-white/5 pt-3 space-y-2">
        <div className="flex items-center justify-between">
          <button className="grid h-9 w-9 place-items-center rounded-lg hover:bg-white/5 transition" title="Notifications" onClick={() => toast("No new notifications 🔔")}>
            <Bell className="h-4 w-4" />
          </button>
          <button className="grid h-9 w-9 place-items-center rounded-lg hover:bg-white/5 transition" title="Explore" onClick={() => toast("Explore coming soon 🧭")}>
            <Compass className="h-4 w-4" />
          </button>
          <button className="grid h-9 w-9 place-items-center rounded-lg hover:bg-white/5 transition" title="Settings" onClick={() => toast("Settings coming soon ⚙️")}>
            <Settings className="h-4 w-4" />
          </button>
          <button onClick={handleLogout} title="Sign out" className="grid h-9 w-9 place-items-center rounded-lg hover:bg-white/5 transition text-muted-foreground hover:text-foreground">
            <LogOut className="h-4 w-4" />
          </button>
        </div>
        {user && (
          <div className="flex items-center gap-2 rounded-xl bg-white/5 px-2 py-2">
            <div className={`grid h-8 w-8 shrink-0 place-items-center rounded-full bg-gradient-to-br ${user.avatarColor} text-xs font-bold text-white`}>
              {user.name[0].toUpperCase()}
            </div>
            <div className="min-w-0 flex-1">
...
/* ─── Trip Workspace ─────────────────────────────── */
function TripWorkspace({ trip, onOpenAI }: { trip: Trip; onOpenAI: () => void }) {
  return (
        ...
        <div className="absolute bottom-6 left-6 right-6 flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="flex -space-x-2">
              {trip.members.map((m) => (
                <div key={m.name} title={m.name} className={`grid h-9 w-9 place-items-center rounded-full border-2 border-background bg-gradient-to-br ${m.color} text-xs font-semibold text-white`}>
                  {m.name[0]}
                </div>
              ))}
              <button title="Invite member" onClick={() => toast("Invite coming soon! 👥")} className="grid h-9 w-9 place-items-center rounded-full border-2 border-background bg-white/10 hover:bg-white/20 transition">
          ...
          trip.days.map((d) => (
            <div key={d.day}>
              <div className="mb-3 flex items-baseline justify-between">
                ...
                <button onClick={() => toast("Add place coming soon! 📍")} className="rounded-full glass px-3 py-1.5 text-xs text-muted-foreground hover:text-foreground hover:bg-white/10 transition">
                  + Add place
                </button>
              </div>
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {d.places.map((p, i) => <PlaceCard key={p.id} place={p} index={i} />)}
              </div>
            </div>
          ))
        )}
    ...
    <div className="glass rounded-3xl p-5">
      ...
      <div className="mt-4 h-2 overflow-hidden rounded-full bg-white/5">
        <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 1, ease: [0.2, 0.8, 0.2, 1] }}
          className="h-full rounded-full" style={{ background: isOver ? "var(--destructive)" : "var(--gradient-aurora)" }} />
      </div>
    </div>
  );
}
...
function MapPanel() {
  ...
  return (
    <div className="glass relative h-72 overflow-hidden rounded-3xl">
      <motion.div key={mapStyle} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}
        className="absolute inset-0" style={{ background: bgByStyle[mapStyle] }} />
      <svg className="absolute inset-0 h-full w-full opacity-50" viewBox="0 0 100 60" preserveAspectRatio="none">
        <defs>
          <pattern id="grid" width="6" height="6" patternUnits="userSpaceOnUse">
            <path d="M6 0H0V6" fill="none" stroke="oklch(1 0 0 / 0.06)" strokeWidth="0.2" />
          </pattern>
          <linearGradient id="routegrad" x1="0" x2="1">
            <stop offset="0" stopColor="oklch(0.72 0.2 295)" />
            <stop offset="1" stopColor="oklch(0.78 0.18 45)" />
          </linearGradient>
        </defs>
      ...
      <div className="absolute left-5 top-5 glass-strong rounded-xl px-3 py-2 text-xs">
        <span className="text-muted-foreground">Route · </span>5 stops · 47 km
      </div>
      <div className="absolute right-5 top-5 flex gap-2">
        {(["satellite", "terrain"] as const).map((style) => (
          <button key={style} onClick={() => setMapStyle(mapStyle === style ? "default" : style)}
            ...
            {style === "satellite" ? <Satellite className="h-3 w-3" /> : <Layers className="h-3 w-3" />}
            {style}
          </button>
        ))}
      </div>
...
function PlaceCard({ place, index }: { place: { id: string; name: string; city: string; emoji: string; tag: string; time: string; image: string }; index: number }) {
  return (
...
function EmptyDays({ onOpenAI }: { onOpenAI: () => void }) {
  return (
    <div className="glass relative overflow-hidden rounded-3xl p-12 text-center">
      ...
      <div className="relative">
        <div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl gradient-aurora shadow-glow"><Wand2 className="h-6 w-6 text-primary-foreground" /></div>
        <h3 className="mt-5 font-display text-3xl">This trip is a blank canvas</h3>
        <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">Let Voyage AI compose your first draft, then make it yours.</p>
        <button onClick={onOpenAI} className="mt-6 inline-flex items-center gap-2 rounded-full px-6 py-3 text-sm font-semibold text-primary-foreground shadow-glow transition hover:scale-[1.02]" style={{ background: "var(--gradient-aurora)" }}>
...
function ChatPanel({ tripTitle, user }: { tripTitle: string; user: AuthUser | null }) {
  const [msgs, setMsgs] = useState([
    ...
    { who: "Voyage AI", text: "I found a quiet rooftop in Pyrgos with a 9.4 sunset rating. Add it?", color: "from-cyan-400 to-violet-500", ai: true },
  ]);
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      setTyping(false);
      setMsgs((prev) => [...prev, { who: "Sana", text: "Let's do it! 🙌", color: "from-sky-400 to-cyan-500" }]);
    }, 3000);
  ...
  const sendMsg = (e: React.FormEvent) => {
    ...
    const color = user?.avatarColor ?? "from-emerald-400 to-teal-500";
    setMsgs((prev) => [...prev, { who: name, text, color }]);
    setVal("");
    if (text.includes("?") || text.toLowerCase().includes("suggest") || text.toLowerCase().includes("add")) {
      setTimeout(() => {
        setMsgs((prev) => [...prev, { who: "Voyage AI", text: "Great idea! Let me look into the best options for that. 🗺️", color: "from-cyan-400 to-violet-500", ai: true }]);
      }, 1200);
    }
  };
    ...
    <aside className="glass hidden w-80 shrink-0 flex-col rounded-3xl p-4 xl:flex h-[calc(100vh-2rem)] sticky top-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm font-semibold truncate max-w-[140px]">{tripTitle}</p>
        </div>
        <div className="flex -space-x-2">
          {["M", "L", "S"].map((c, i) => (
            <div key={i} className="grid h-7 w-7 place-items-center rounded-full border-2 border-background gradient-aurora text-[11px] font-semibold">{c}</div>
          ))}
        </div>
      </div>
      ...
      <div className="mt-4 flex-1 space-y-3 overflow-y-auto pr-1">
        {msgs.map((m, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="flex gap-2">
            <div className={`grid h-7 w-7 shrink-0 place-items-center rounded-full bg-gradient-to-br ${m.color} text-[10px] font-semibold`}>
              {(m as { ai?: boolean }).ai ? "✨" : m.who[0]}
            </div>
      ...
      <form onSubmit={sendMsg} className="mt-3 flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 p-1.5">
        ...
        <button type="submit" disabled={!val.trim()} className="grid h-8 w-8 place-items-center rounded-xl text-primary-foreground shadow-glow transition disabled:opacity-50 disabled:shadow-none" style={{ background: "var(--gradient-aurora)" }}>
...
/* ─── AI Assistant Modal ─────────────────────────── */
function AIAssistantModal({ onClose, onAddToTrip, hasDays }: { onClose: () => void; onAddToTrip: (days: Day[]) => void; hasDays: boolean }) {
  const [stage, setStage] = useState<"idle" | "thinking" | "done">("idle");
  const [prompt, setPrompt] = useState("5 days in Greece, romantic, slow mornings, sunsets");

  const generate = () => { if (!prompt.trim()) return; setStage("thinking"); setTimeout(() => setStage("done"), 2200); };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.95, y: 20, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
        ...
        <div aria-hidden className="absolute -right-20 -top-20 h-72 w-72 rounded-full opacity-50 blur-3xl gradient-aurora" />
        ...
        <div className="relative">
          <div className="inline-flex items-center gap-2 rounded-full glass px-3 py-1 text-xs"><Sparkles className="h-3.5 w-3.5 text-accent" /> Voyage AI</div>
          <h2 className="mt-4 font-display text-4xl">Compose your trip</h2>
          <p className="mt-1 text-sm text-muted-foreground">Describe a feeling. The AI handles the rest.</p>
          ...
          {hasDays && stage === "idle" && (
            <div className="mt-3 flex items-center gap-2 rounded-xl border border-accent/20 bg-accent/5 px-3 py-2 text-xs text-muted-foreground">
              ...
              This trip already has an itinerary. Generating will replace it.
            </div>
          )}

          <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} rows={3} disabled={stage === "thinking"}
            className="mt-5 w-full resize-none rounded-2xl border border-white/10 bg-white/5 p-4 text-sm placeholder:text-muted-foreground focus:border-primary/50 focus:outline-none disabled:opacity-60" />

          {stage === "idle" && (
            <button onClick={generate} disabled={!prompt.trim()}
              className="mt-4 inline-flex items-center gap-2 rounded-full px-6 py-3 text-sm font-semibold text-primary-foreground shadow-glow transition hover:scale-[1.02] disabled:opacity-50 disabled:hover:scale-100 disabled:shadow-none"
              style={{ background: "var(--gradient-aurora)" }}>
            ...
            <div className="mt-6 space-y-3">
              <div className="flex items-center gap-3 text-sm text-muted-foreground">
                <div className="relative h-8 w-8">
                  <div className="absolute inset-0 rounded-full gradient-aurora blur-md opacity-70 animate-pulse" />
              ...
              {["Mapping golden-hour windows", "Pairing villages with quiet mornings", "Booking-ready dinner spots"].map((s, i) => (
                <motion.div key={s} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.5 }} className="glass rounded-xl px-4 py-2.5 text-sm">
          ...
          {stage === "done" && (
            <div className="mt-6 space-y-2">
              {AI_GENERATED_DAYS.map((day, i) => (
                <motion.div key={day.day} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} className="glass flex items-center gap-3 rounded-2xl p-3">
                  <div className="grid h-10 w-10 place-items-center rounded-xl bg-white/5 text-lg">{day.places[0]?.emoji ?? "✨"}</div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] uppercase tracking-wide text-muted-foreground">Day {day.day}</p>
                    <p className="text-sm truncate">{day.title}</p>
                  </div>
                  <p className="text-[10px] text-muted-foreground shrink-0">{day.places.length} stop{day.places.length !== 1 ? "s" : ""}</p>
                </motion.div>
              ))}
              <div className="flex gap-2 pt-2">
                <button onClick={() => onAddToTrip(AI_GENERATED_DAYS)}
                ...
                <button onClick={() => setStage("idle")} className="rounded-full glass px-5 py-3 text-sm hover:bg-white/10 transition">Refine</button>
              </div>
            </div>
          )}
        </div>
...
/* ─── New Trip Modal ─────────────────────────────── */
function NewTripModal({ onClose, onCreate }: { onClose: () => void; onCreate: (data: { title: string; destination: string; start: string; end: string; budget: number }) => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.form onClick={(e) => e.stopPropagation()} onSubmit={submit}
        ...
        <div className="flex items-center gap-3">
          <div className="grid h-11 w-11 place-items-center rounded-2xl gradient-aurora shadow-glow"><Sparkles className="h-5 w-5 text-primary-foreground" /></div>
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">New journey</p>
            <h2 className="font-display text-2xl">Plan a new trip</h2>
          </div>
        </div>
        <div className="mt-6 space-y-4">
          <Field label="Trip name *">
            <input autoFocus value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Sunset in Lisbon" maxLength={60}
              className="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-sm placeholder:text-muted-foreground focus:border-primary/50 focus:outline-none" />
          </Field>
          <Field label="Destination *">
            <input value={destination} onChange={(e) => setDestination(e.target.value)} placeholder="Portugal" maxLength={60}
              className="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-sm placeholder:text-muted-foreground focus:border-primary/50 focus:outline-none" />
          </Field>
          <div className="grid grid-cols-2 gap-3">
            <Field label="Start date *">
              <div className="relative">
                ...
                <input type="date" value={start} min={today} onChange={(e) => { setStart(e.target.value); if (end && e.target.value > end) setEnd(""); }}
                  className="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-sm focus:border-primary/50 focus:outline-none [color-scheme:dark]" />
              </div>
            </Field>
          ...
          <Field label="Budget (€)">
            <input type="number" min={1} max={999999} value={budget} onChange={(e) => setBudget(e.target.value)} placeholder="2000"
              className="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-sm placeholder:text-muted-foreground focus:border-primary/50 focus:outline-none" />
          </Field>
        </div>
        <AnimatePresence>
        ...
        <div className="mt-6 flex items-center justify-end gap-2">
          ...
          <button type="submit" disabled={loading}
            className="inline-flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-semibold text-primary-foreground shadow-glow transition hover:scale-[1.02] active:scale-[0.99] disabled:opacity-70 disabled:hover:scale-100 disabled:shadow-none"
            style={{ background: "var(--gradient-aurora)" }}>
